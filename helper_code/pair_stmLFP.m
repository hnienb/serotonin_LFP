...
for d = 1:2
    ...
    for i = 1:lenv    
        for n = 1:para.cond(d).ntr(i)
            para.cond(d).trials{i}(n).(lfpfield) = (para.cond(d).trials{i}(n).(lfpfield) - me(1))/sd(1);
...
ntr = length(spk);
% LFP values within analysis window
...
% Randomly select out spikes until matches firing rate ratio
r = rand(1, length(spk));
spk(r <= 1 - fr_ratio) = []; 
