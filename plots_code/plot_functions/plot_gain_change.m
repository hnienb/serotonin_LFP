...
if sum(contains(analysis, 'all'))==1 || sum(contains(analysis, 'Loading'))==1
    ...
    % List of files that already included in the "all" versions of the same
    % sessions, feel free to add more as needed
    remove = ["ma_0188_c1_sortLH_12.34PM.grating.ORxRC.mat", ...
        "ma_0205_c1_sortLH_1.08PM.grating.ORxRC.mat", ...
        "ma_0188_c1_sortLH_12.41PM.grating.ORxRC_5HT.mat", ...
        "ma_0205_c1_sortLH_1.14PM.grating.ORxRC_5HT.mat"];

    % Remove any files that do not contain RC to speed up process
    removal_indices = zeros(1, length(filenames));
    for index = 1:length(filenames)
        % Remove non-RC files from consideration
        if ~contains(filenames(index), 'RC')
            removal_indices(index) = 1; 
        end
        % Remove redundant data files
        if ismember(filenames(index), remove)
            removal_indices(index) = 1;
        end
    end
    filenames(removal_indices == 1) = [];
    ...
    parfor file_index = 1:length(filenames)
        ...
        if ~contains(file{:}, '5HT') && ~contains(file{:}, 'NaCl')
            ...
            try
                ...
                ex_base = loadCluster(file_base, 'loadlfp', false);
                ...
                for drug_index = 1:length(filenames)
                    ...
                    if contains(drug_name{:}, session_name) && ~isequal(drug_name, file)
                        ...
                        ex_drug = loadCluster(file_drug, 'loadlfp', false);
...
if sum(contains(analysis, 'all'))==1 || sum(contains(analysis, 'Subspace'))==1
    ...
    parfor ex_index = 1:length(ex_results)
